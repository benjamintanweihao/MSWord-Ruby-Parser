...
FILE_TO_PARSE = "C:\\Documents\ and\ Settings\\Ben\\Desktop\\test.htm"
TEMPLATE_FILE = "C:\\Documents\ and\ Settings\\Ben\\Desktop\\template.htm"
START_DELIMITER = "###"
BODY_DELIMITER = "## BODY ##"
...
def get_template_html( file_path )
	file_contents = ""
...
def get_assembled_question( template_html, images_directory, question_html )
	assembled_question = ""
...
def write_question_to_file( file_name, question)
	File.open( file_name+".htm", 'w') {|f| f.write( question )}
end

questions_array = parse_questions( FILE_TO_PARSE )
template_html = get_template_html( TEMPLATE_FILE )

questions_array.each_with_index do |question, index|
	assembled_question = get_assembled_question( template_html, './Maths Mid term test_files', question )
	write_question_to_file( "Maths_Mid_Term_Qn_"+index.to_s, assembled_question);
end